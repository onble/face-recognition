...
function render_page_num_with_name(page, all_pages) {
    // 绘制页码导航
    // page变量从0开始计数
    ...
    function click_page_num(element) {
        // TODO: 这个关于按照姓名查询页码数据监听的还需要再测试,等插入更多的数据
        ...
        if (innerHTML == "下一页") {
            ...
            getNamePage(parseInt(element.innerHTML));
            return;
        }
    }
    // 获取盒子
...
function reder_date(data) {
    ...
    const box = document.querySelector(
        "body>div.x-body>table.layui-table>tbody"
    );
    clearbox(box);
    let result = ``;
    // 遍历数据，逐条渲染
...
function render_inf(page) {
    ...
    // 2.初始化 设置类型与URL
    xhr.open("POST", domain + "/phone_list/getList");
    // 3.发送
...
function getNamePage(page) {
    ...
    const xhr = new XMLHttpRequest();
    // 2.初始化 设置类型与URL
    xhr.open("POST", domain + "/phone_list/getListWithName");
    // 获取formDate
