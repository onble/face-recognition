...
function reder_edit_data(data) {
    ...
    // 修改内容
    const homePage_input = document.querySelector("textarea[name='duty']");
    homePage_input.value = data["duty"];
}
...
function init() {
    ...
    // 将id传送给输入框存储
    document.querySelector("input[name=id]").value = getQueryVariable("id");
    // 后台成功接收到传输的数据
    // 4.事件绑定
