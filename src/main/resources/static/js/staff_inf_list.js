...
function reder_date(data) {
    ...
    data.forEach(function (value, index) {
        ...
        <tr>
        <td>
            <input type="checkbox" value="${value["staffId"]}" name="id" />
        </td>
        <td>${value["name"]}</td>
        <td>${value["age"]}</td>
        <td>${value["phone"]}</td>
        ...
        <td>${value["monthlySalary"]}元</td>
        <td>${value["departmentId"]}</td>
        <td>${value["birthday"]}</td>
        <td>${value["folderSize"]}Byte</td>
        <td>${value["occupyFileSize"]}Byte</td>

        <td class="td-manage">
            <a
                title="编辑"
                href="javascript:;"
                onclick="todo_edit('编辑','todoadd.html','${value["staffId"]}','1000','600')"
                class="ml-5"
            ...
            <a
                title="删除"
                href="javascript:;"
                onclick="todo_del(this,'${value["staffId"]}')"
                style="text-decoration: none"
            >
                <i class="layui-icon">&#xe640;</i>
            </a>
...
function render_inf(page) {
    ...
    // 2.初始化 设置类型与URL
    xhr.open("POST", domain + "/staffInf/getList");
    // 3.发送
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                // 重新渲染页面数据
                reder_date(result["staffInf_inf"]);
