...
public interface MeetingInfMapper {
    ...
    @Select("SELECT meeting.title,meeting.address,meeting.content,meeting.meeting_id,meeting.meeting_id,meeting_inf.`name`,meeting.start_time,meeting.stop_time,meeting.`status` FROM meeting,meeting_inf WHERE meeting_inf.meeting_id=meeting.meeting_id")
...
//    List<Department> selectAll() throws Exception;
    ...
    // 修改数据
    @Update("update meeting set title=#{title},address=#{address},content=#{content},staffId=#{satffId},startTime=#{startTime},stopTime=#{stopTime},status=#{status} where id=#{meetingId}")
    ...
    // 获取数据数量
    @Select("select count(id) from meeting where meeting_id =#{meetingId}")
