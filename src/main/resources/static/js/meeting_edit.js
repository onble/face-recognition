...
function reder_edit_data(data) {
    ...
    // 修改开始时间
    const startTime_input = document.querySelector("input[name='startTime']");
    startTime_input.value = data["startTime"];
    // 修改结束时间
    const stopTime_input = document.querySelector("input[name='stopTime']");
    stopTime_input.value = data["stopTime"];
    // 修改状态
    if (data["status"] == true) {
        // 去选择已完成
...
function init() {
    ...
    // 2.初始化 设置类型与URL
    xhr.open("POST", domain + "/meeting/get");
    // 3.发送
    ...
    // 将id传送给输入框存储
    document.querySelector("input[name=meeting_id]").value =
        getQueryVariable("id");
    // 后台成功接收到传输的数据
    // 4.事件绑定
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                // 重新渲染页面数据
                reder_edit_data(result["meeting_inf"]);
            }
        }
    };
}
